package com.sampson.fullstack.exception;
...
public class RestExceptionHandler implements ExceptionMapper<HibernateException> {
    ...
    private final static String PG_UNIQUE_VIOLATION_ERROR = "23505";
    ...
    public Response toResponse(HibernateException exception) {
        ...
        if (hasExceptionInChain(exception, ConstraintViolationException.class) || hasPostgresErrorCode(exception, PG_UNIQUE_VIOLATION_ERROR)){
    ...
    private static boolean hasPostgresErrorCode(Throwable throwable, String code) {
        return getExceptionInChain(throwable, PgException.class).filter(ex -> Objects.equals(ex.getCode(), code)).isPresent();
    }
